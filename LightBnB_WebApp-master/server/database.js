...
const getAllProperties = function (options, limit = 10) {
 ...
 `;
  let paramsChecker = false;
  // 3

  if (options.city) {
    queryParams.push(`%${options.city}%`);
    queryString += `${paramsChecker ? 'AND' : 'WHERE'} city LIKE $${queryParams.length} `;
    paramsChecker = true;
  }

  if(options.owner_id){
    queryParams.push(`${options.owner_id}`);
    queryString += `${paramsChecker ? 'AND' : 'WHERE'} owner_id = $${queryParams.length} `;
    paramsChecker = true;
  }

  if (options.minimum_price_per_night) {
    queryParams.push(options.minimum_price_per_night * 100);
    queryString += `${paramsChecker ? 'AND' : 'WHERE'} cost_per_night >= $${queryParams.length} `;
    paramsChecker = true;
  }
